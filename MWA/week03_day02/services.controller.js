...
module.exports.addService = (req, resp) => {
  ...
  if (req.body.imageUrl)
    if (req.body.rate) service.rate = parseFloat(req.body.rate);
...
module.exports.getOneService = (req, resp) => {
  ...
  find
    .then((service) => _OnFindByIdFucation(service, resp))
    .catch((err) => {
      _out(resp, err, null);
    });
};
...
module.exports.editService = (req, resp) => {
  ...
  const service = {};
  if (req.body.name) service.name = req.body.name;
  if (req.body.imageUrl) service.imageUrl = req.body.imageUrl;
  if (req.body.rate) service.rate = parseFloat(req.body.rate);
