...
console.log(
    arr.filter((n) => n > 20).reduce((sum, current) => (sum += current), 0)
);
// Question 2
const arrOfStrings = ["Kareem 1", "Kareem 2", "Kareem 3", "Kareem 4", "Kareem 5", "Kareem 6"];
console.log(arrayStrings.filter((s) => s.length >= 5 && s.includes("a")));
// Question 3
...
this.getAge = function() {
    ...
    if (
        currentMonth < 0 ||
        (currentMonth === 0 && currentDate.getDate() < this.birthdate.getDate())
    ) {
        age--;
    }
    return age;
};
const listOfEmplyees = [
    new Employee("Kareem 1", "AbuRejila 1", new Date(1996, 28, 11)),
